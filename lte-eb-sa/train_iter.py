...
def main(config_, save_path):
    ...
    num_iter_per_epoch = math.ceil(
        len(train_loader.dataset) / config['train_dataset']['batch_size'])
    ...
    config['multi_step_lr']['milestones'] = [math.floor(
        milestone / num_iter_per_epoch) for milestone in config['multi_step_lr']['milestones']]
    ...
    for epoch in range(epoch_start, epoch_max + 1):
        ...
        if (epoch_val is not None) and (epoch % epoch_val == 0):
            ...
            if val_res > max_val_v:
                max_val_v = val_res
                torch.save(sv_file, os.path.join(
                    save_path, 'iteration-best.pth'))
