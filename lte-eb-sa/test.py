...
def save_images_to_dir(out_dir, inp, pred, gt, step=0):
    ...
    transforms.ToPILImage()(pred[0]).save(
        f'{out_dir}/img-{step}.png')
...
def eval_psnr(loader, model, data_norm=None, eval_type=None, eval_bsize=None, max_scale=4,
              verbose=False, device="cuda", out_dir=None, window_size=0):
    model.eval()
    ...
    if eval_type is None:
        ...
        metric_lpips = partial(
            utils.calc_lpips, dataset='div2k', scale=scale, device=device)
    elif eval_type.startswith('benchmark'):
        scale = int(eval_type.split('-')[1])
        metric_psnr = partial(
            utils.calc_psnr, dataset='benchmark', scale=scale)
        metric_ssim = partial(
            utils.calc_ssim, dataset='benchmark', scale=scale)
        metric_lpips = partial(
            utils.calc_lpips, dataset='benchmark', scale=scale, device=device)
    else:
    ...
    for i, batch in enumerate(pbar):
        ...
        if window_size != 0:
            ...
            inp = torch.cat([inp, torch.flip(inp, [2])], 2)[
                :, :, :h_old + h_pad, :]
            inp = torch.cat([inp, torch.flip(inp, [3])], 3)[
                :, :, :, :w_old + w_pad]

            coord = utils.make_coord(
                (scale*(h_old+h_pad), scale*(w_old+w_pad))).unsqueeze(0).cuda()
        ...
        if eval_type is not None:  # reshape for shaving-eval
            ...
            pred = pred.reshape(
                pred.shape[0], sample_patch_size, sample_patch_size, 3).permute(0, 3, 1, 2)
            batch['gt'] = batch['gt'].reshape(
                pred.shape[0], sample_patch_size, sample_patch_size, 3).permute(0, 3, 1, 2)

        if out_dir is not None:
            save_images_to_dir(
                out_dir, batch['inp'], pred, batch['gt'], step=i)
        ...
        if verbose:
            pbar.set_description('val psnr {:.4f} | ssim loss {:.4f} | lpips loss {:.4f}'.format(
                val_res_psnr.item(), val_res_ssim.item(), val_res_lpips.item()))
